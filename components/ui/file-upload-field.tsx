    ...
    <div className="space-y-2 text-left">
      ...
      <div className="relative flex items-center gap-2">
        ...
        <input
          ...
          className="w-full bg-[#F7F9FC] border border-slate-200 rounded-xl pl-9 pr-16 py-2 text-slate-800 outline-none focus:border-primary font-mono text-[11px] placeholder:text-slate-500 shadow-2xs"
        />
        <div className="absolute right-2 flex items-center gap-1">
          {value && (
            <>
              ...
              <button type="button" onClick={handleClear} className="text-slate-500 hover:text-red-500 transition-colors cursor-pointer" title="Effacer">
      ...
      {preview === "image" && value && value.match(/\.(jpg|jpeg|png|gif|webp|svg)(\?|$)/i) && (
        <div className="relative w-full h-28 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center p-1.5 group">
          ...
          />
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-white backdrop-blur-xs transition-opacity rounded-xl">
            <a href={value} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 rounded-lg bg-white/90 hover:bg-white text-slate-900 text-xs font-bold flex items-center gap-1.5 shadow-xs">
