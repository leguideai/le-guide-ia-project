...
export function Pricing({ selectedCourseId, courses, activeCourse: propActiveCourse, onSelectCourse }: PricingProps) {
      ...
      <section className="py-16 bg-background relative overflow-hidden border-t border-border/50 animate-pulse" id="tarifs">
        <div className="mx-auto max-w-7xl px-4 md:px-8 space-y-8">
          <div className="space-y-3">
            ...
            <div className="h-9 w-80 bg-[#EEF3FF] rounded-xl" />
          </div>
          <div className="grid gap-6 md:grid-cols-2 max-w-5xl mx-auto">
            <div className="h-96 rounded-3xl border border-slate-200 bg-card/40" />
            <div className="h-96 rounded-3xl border border-slate-200 bg-card/40" />
          </div>
        </div>
      </section>
  ...
  const theme = isBusiness
    ? {
        border: "border-2 border-[#D4AF37] glow-gold bg-white",
        badge: "bg-gradient-to-r from-[#D4AF37] via-[#F3E5AB] to-[#AA7C11] text-slate-950 border border-[#F3E5AB]",
        priceColor: "text-[#0E1E3F]",
        expireColor: "text-[#8A6A1F]",
        expireIcon: "text-[#8A6A1F]",
        checkColor: "text-[#8A6A1F]",
        btn: "bg-[#D4AF37] hover:bg-[#c49f2c] text-slate-950 font-black shadow-sm shadow-[#D4AF37]/25",
      }
    : {
        border: "border-2 border-primary glow-blue bg-white",
        badge: "bg-primary text-slate-950 border border-amber-300 font-black",
        priceColor: "text-[#0E1E3F]",
        expireColor: "text-amber-700",
        expireIcon: "text-primary",
        checkColor: "text-primary",
        btn: "bg-primary hover:bg-primary/90 text-slate-950 font-black shadow-sm shadow-primary/25",
      }
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8 space-y-8">
        ...
        <div className="space-y-4 text-left">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <span className={`inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-widest px-3.5 py-1.5 rounded-full border ${isBusiness ? "text-[#8A6A1F] bg-[#D4AF37]/10 border-[#D4AF37]/30" : "text-primary bg-primary/10 border-primary/20"}`}>
        ...
        <div className="grid gap-8 md:grid-cols-2 max-w-5xl mx-auto items-stretch pt-6 md:pt-8">
          ...
          <motion.div
            ...
            className={`relative rounded-3xl p-6 md:p-8 shadow-xs flex flex-col justify-between z-10 transition-all ${
              isOfferExpired 
                ? "border border-border/60 bg-card/20 opacity-75 grayscale-[0.4]" 
                : isEnrolled
                  ? "border-2 border-emerald-500/60 bg-[#F7F9FC] shadow-emerald-500/10 shadow-xs"
                  : isPending
                    ? "border-2 border-amber-500/60 bg-[#F7F9FC] shadow-amber-500/10 shadow-xs"
                    : theme.border
            }`}
          >
            {/* Top Badge */}
            <div className={`absolute -top-3.5 left-1/2 -translate-x-1/2 rounded-full px-5 py-1 text-xs font-black uppercase tracking-widest flex items-center gap-1.5 shadow-sm z-20 ${
              isEnrolled
                ...
                : isPending
                  ...
                  : isOfferExpired 
                    ? "bg-[#EEF3FF] text-slate-500 border border-slate-300" 
                    : theme.badge
            }`}>
            ...
            <div>
              <div className="text-center mt-4 mb-6 space-y-3">
                ...
                {isEnrolled && (
                  <div className="py-2 px-3 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-600 text-xs font-bold flex items-center justify-center gap-2">
                ...
                {isPending && (
                  <div className="py-2 px-3 rounded-2xl bg-amber-500/15 border border-amber-500/30 text-amber-700 text-xs font-bold flex items-center justify-center gap-2">
                ...
                {!isEnrolled && !isPending && hasOfferEndDate && !isOfferExpired && (
                  <div className={`py-2 px-3 rounded-2xl border text-xs font-mono font-bold flex items-center justify-center gap-2 ${
                    isBusiness ? "bg-[#D4AF37]/15 border-[#D4AF37]/40 text-[#8A6A1F]" : "bg-primary/15 border-primary/30 text-amber-700"
                  }`}>
                    <Clock className="size-4 animate-pulse shrink-0 text-primary" />
                    <span>
                      Fin de l'offre : <strong className="text-[#0E1E3F]">{timeLeft.days}j {timeLeft.hours}h {timeLeft.minutes}m {timeLeft.seconds}s</strong>
                    </span>
                  </div>
                )}

                {!isEnrolled && !isPending && hasOfferEndDate && isOfferExpired && (
                  <div className="py-2 px-3 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-600 text-xs font-bold flex items-center justify-center gap-2">
                ...
                <ul className="space-y-3.5">
                  {current.features.map((f: string, index: number) => (
                    <li key={index} className={`flex items-start gap-3 text-xs md:text-sm ${isOfferExpired ? "text-muted-foreground" : "text-foreground/95"}`}>
                      <Check className={`size-4 shrink-0 mt-0.5 ${isEnrolled ? "text-emerald-600" : isPending ? "text-amber-700" : isOfferExpired ? "text-slate-600" : theme.checkColor}`} />
            ...
            <div className="mt-8">
              {isEnrolled ? (
                <Link
                  href="/dashboard"
                  className="w-full flex h-13 items-center justify-center gap-2 rounded-xl text-xs sm:text-sm font-black bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm shadow-emerald-600/20 active:scale-95 transition-all cursor-pointer"
                >
                ...
                <Link
                  href="/dashboard"
                  className="w-full flex h-13 items-center justify-center gap-2 rounded-xl text-xs sm:text-sm font-black bg-amber-500 hover:bg-amber-400 text-slate-950 shadow-sm shadow-amber-500/25 active:scale-95 transition-all cursor-pointer"
                >
                ...
                <Link
                  ...
                  className={`w-full flex h-13 items-center justify-center gap-2 rounded-xl text-sm transition-all shadow-sm active:scale-95 cursor-pointer ${theme.btn}`}
                >
                ...
                <button
                  type="button"
                  disabled
                  className="w-full flex h-13 items-center justify-center gap-2 rounded-xl text-xs sm:text-sm bg-[#EEF3FF] text-slate-500 font-bold border border-slate-300 cursor-not-allowed select-none"
                >
          ...
          <motion.div
            ...
            className={`relative rounded-3xl p-6 md:p-8 shadow-sm flex flex-col justify-between transition-all ${
              isOfferExpired && !isEnrolled && !isPending
                ? "border-2 border-primary glow-gold bg-white shadow-xs z-20" 
          ...
          >
            ...
            <div>
              <div className="text-center mt-4 mb-6 space-y-3">
                <div className="flex items-center justify-center gap-2 pt-2">
                  <span className={`font-heading text-4xl md:text-5xl font-black ${isOfferExpired ? "text-[#0E1E3F]" : "text-foreground"}`}>
                    {current.standardPrice}
                  </span>
                </div>

                {isOfferExpired && !isEnrolled && !isPending && (
                  <div className="py-2 px-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 text-xs font-bold flex items-center justify-center gap-2">
            ...
            <div className="mt-8">
              {isEnrolled ? (
                <Link
                  href="/dashboard"
                  className="w-full flex h-13 items-center justify-center gap-2 rounded-xl text-xs sm:text-sm font-bold bg-emerald-600/20 border border-emerald-500/40 text-emerald-700 hover:bg-emerald-600/30 active:scale-95 transition-all cursor-pointer"
                >
                ...
                <Link
                  href="/dashboard"
                  className="w-full flex h-13 items-center justify-center gap-2 rounded-xl text-xs sm:text-sm font-bold bg-amber-500/20 border border-amber-500/40 text-amber-700 hover:bg-amber-500/30 active:scale-95 transition-all cursor-pointer"
                >
                  <Clock className="size-4 animate-pulse text-amber-700" />
                ...
                <Link
                  ...
                  className={`w-full flex h-13 items-center justify-center gap-2 rounded-xl font-bold text-xs md:text-sm transition-all active:scale-95 cursor-pointer shadow-sm ${
                    isOfferExpired 
